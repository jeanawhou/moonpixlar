...
function ihc_set_trial_time_for_stripe($level_data, $l_id, $u_id){
	/*
	...
	if ($u_id && $notification_type){
		...
		if (!empty($data)){
			...
			if ($subject && $message && $user_email){
				...
				$admin_case = array('admin_user_register', 'admin_user_expire_level', 'admin_before_user_expire_level', 'admin_user_payment', 'admin_user_profile_update');
...
function ihc_get_payment_id_by_order_id($order_id=0){
	...
	if ($order_id){
		global $wpdb;
		$p = $wpdb->base_prefix . 'indeed_members_payments';
		$o = $wpdb->base_prefix . 'ihc_orders';
